...
where $P$ is the operator that permute the index of the particles. $(-1)^P$ means we consider the corresponding coefficient according to the even/oddness of the permutation. A permutation $P$ is even if it can be transformed into identity by even number of nearest neighbor swap. For example, for the permutation $P^*$ such that $P^*(ABC)=()$
