...
That is, if we have a $M$-dimensional single particle Hilbert space $\mathcal{H}_1$ labeled by quantum number $a_j;j=1\sim M$, we can have our fictitious state described by $|\{\alpha_i\})$ where $\alpha_i\in\{a_j\}$. Then, we can have the occupation number representation of a state as $|n_{a_1}n_{a_2}...n_{a_M}\rangle$ where $\sum_jn_j=N$. If $M$ is unbounded, we usually have the occupation number representation denoted as $|n_{a_1}n_{a_2}...\rangle$ without specifying the occupation number of the last state since it is not 
