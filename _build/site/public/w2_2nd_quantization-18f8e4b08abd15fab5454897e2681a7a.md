...
Here, we assume the compelteness relation in $\mathcal{H}_1$ as $\sum_{\alpha}|\alpha\rangle\langle\alpha|=\textbf{1}_{\mathcal{H}_1}$. Also, notice that this state is *NOT* the physical wave function of $N$ identicle particles. This state is just a intermediate devise for us to construct and understand the 
...
where $P$ is the operator that permute the index of the particles.
